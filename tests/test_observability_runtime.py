...
def test_api_policy_history_and_rollback(monkeypatch, tmp_path):
    ...
    # Isolate mutable globals for this test.
    monkeypatch.setattr(app_module, "ops_store", OpsStore(str(tmp_path / "ops_e.db")))
    ...
    hist = client.get("/api/policy/history?limit=10")
    assert hist.status_code == 200
    hdata = hist.get_json()
    assert hdata["count"] >= 2

    rollback = client.post("/api/policy/rollback", json={"to_version": 1})
    assert rollback.status_code == 200
    rdata = rollback.get_json()
    assert rdata["policy"]["mode"] == "monitor"


def test_api_siem_flush(monkeypatch):
    import web_app.app as app_module
    from src.observability.siem_exporter import SiemExporter
    ...
    client = app_module.app.test_client()
    resp = client.get("/api/siem/flush?limit=10")
...
def test_api_detect_updates_engine_metrics(monkeypatch):
    import web_app.app as app_module
    ...
    client = app_module.app.test_client()
    resp = client.post("/api/detect", json={"source": "10.0.0.1", "features": {"src_bytes": 10, "dst_bytes": 2}})
    assert resp.status_code == 200
